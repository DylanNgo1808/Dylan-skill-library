...
Seven [Claude Code skills](https://docs.claude.com/en/docs/claude-code/skills) for the shape of work that happens before and around writing code: aligning on what to build, writing it down, generating a build loop to execute it, reasoning/deliberation tools, and local video analysis.

They're plain markdown skill definitions. Drop a folder in, and Claude picks it up automatically when the conversation matches its description. Most require no external service; `video-analysis` uses the Gemini API plus local video tools.
